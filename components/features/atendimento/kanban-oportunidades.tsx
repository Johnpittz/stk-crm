...
export function KanbanOportunidades({
  ...
  const [oportunidades, setOportunidades] = useState<Tarefa[]>([]);
  const [loading, setLoading] = useState(true);
  const [oportunidadeSelecionada, setOportunidadeSelecionada] = useState<Tarefa | null>(null);
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (novaColuna === "comissao_paga") {
      const oportunidadeArrastada = tarefas.find((t) => t.id === draggableId);
    ...
    const oportunidadesNaColunaDestino = tarefas.filter((t) => t.coluna_kanban === novaColuna);
  ...
  const oportunidadesFiltradas = tarefas.filter((t) => {
  ...
  return (
    <Card className="h-full flex flex-col border-[#1c2e4a] bg-[#0c1426]">
                    ...
                    >
                      ...
                      <Droppable droppableId={coluna.id}>
                        {(provided, snapshot) => (
                          ...
                          >
                            {oportunidadesColuna.map((tarefa, index) => (
                              <Draggable
                                key={oportunidade.id}
                              ...
                              >
                                {(provided, snapshot) => (
                                  <div
                                    ...
                                    onClick={() => {
                                      setOportunidadeSelecionada(tarefa);
                                      setModalAberto(true);
                                    }}
                                    className={cn(
      ...
      <ModalDetalhesOportunidade
        tarefa={oportunidadeSelecionada}
        aberto={modalAberto}
        onClose={() => {
