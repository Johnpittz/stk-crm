...
### 6.1 Defeitos de produção pós-cutover (26/09) e suas causas-raiz
1. **Anexo virava DUAS mensagens** — `chat-inline.enviarArquivo` manda o arquivo em `/api/send/media` e depois grava em `/api/atendimentos/mensagens`, cuja rota TAMBÉM chamava `enviarTexto(conteudo)` → cliente recebia o arquivo + um texto de verdade `[document]`/`[Áudio]` e o banco ficava com 2 linhas. Regra: **`media_url` presente ⇒ a rota NÃO envia** (o arquivo já saiu). A rota agora aceita `whatsapp_message_id` vindo do cliente para dedup exato.
2. **Texto duplicava por corrida** — a rota insere a linha SEM `whatsapp_message_id`, envia e grava o id depois; o eco `message.any` costuma chegar nessa janela e o dedup por id não acha nada. Correção no webhook: **dedup por CONTEÚDO** (remetente=vendedor, ≤60s, id do candidato vazio ou igual) com resposta `action: "duplicate_content"`.
3. **Documento baixava como `.bin`** — dois defeitos somados: (a) upload do webhook usava `application/octet-stream` e `getExtensionFromMime` casava xlsx em `document` → virava `.docx`; (b) o link apontava direto para a URL do objeto no Storage e o navegador salvava com o nome do objeto (`1790391732596-r7mxiu.bin`). Correções: `extensaoArquivo()` com fallback pelo **nome original do arquivo** + `/api/media-download` servindo o binário com `Content-Disposition: attachment; filename="<file_name>"`.
4. **"Mensagem enviada para um número chega em outro" é FÍSICO, não bug** — quando os dois lados da conversa são números conectados no próprio WAHA (ex.: STK-1 `556295094949` ↔ STK-3 `556299190117`), a mensagem existe nos dois atendimentos por quê ela realmente viajou de uma conta do WhatsApp para outra. Sinal para distinguir de bug real: o lado "recebido" tem `remetente: cliente` e o id final igual com prefixo `false_…@lid`/`false_…@c.us` vs `true_…`.
